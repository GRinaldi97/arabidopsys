...
def alg_execution():
    ...
    timeline = [root]
    dt = 0.05
    timerange = np.arange(0,120.00,dt)
    for t in timerange:
        root_step = dt*update_grid_cells(root,pins,D=0.5,T=0.4, auxin_source=1)
...
with open('Forward_Euler_time.txt', 'w+') as time_log:
    time_log.write(f"Execution time {end_time-start_time}")



with open('Forward_Euler_CPU_Usage.txt', "w+") as CPU_log:
...
with open("Forward_Euler_Memory.txt", "w+") as Memory_logs:
...
### generate first plot of cpu usage
